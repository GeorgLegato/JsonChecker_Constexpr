...
struct JSON_checkerT {
	...
	constexpr JSON_checkerT() : state(GO), top(-1)  {
		push(MODE_DONE);
	}
...
private:
  ...
  static constexpr const classes ascii_class[128] = {
    ...
    static constexpr const states state_transition_table[NR_STATES][NR_CLASSES] = {
     /*                 white                                      1-9                                   ABCDF  etc
...
constexpr bool operator""_jsonchecker(const char* js, ARCH_INT_TYPE len) {
	JSON_checkerT<20> jc;
	return jc(js, len);
}
